...
$$L(\theta) = \frac{1}{N} \sum_{n=1}^{N} \sum_{t=0}^{T} \beta^t (d_{\pi,t})^T u(c_t)$$
...
for t in range(T_horizon):
    ...
    # 3. 计算效用并累加
    L_n += β^t * (d @ u_torch(c_t))
    
    # 4. 更新状态分布
    A = build_A_pi(theta, iz, ir, ...)  # 转移矩阵
    d = A.T @ d  # 下一期的状态分布
...
```python
def aggregate_saving_grid(theta, d, iz, ir, ...):
    """计算聚合储蓄"""
    c = theta_to_consumption_grid(theta, ...)[:, iz, ir]
    b_next = (1 + r) * b_grid + y_grid * z - c
    return (d @ b_next)  # 加权平均
```

### 5.2 价格确定

在 Huggett 模型中，债券净供给 `B = 0`，因此：

```python
def P_star_detach(theta, d, iz, ...):
    """找到使聚合储蓄 = 0 的利率"""
    for ir in range(nr_spg):
        S = aggregate_saving_grid(theta, d, iz, ir, ...)
        if abs(S - 0) < best_err:
            best_ir = ir
