...
消费策略采用 **4D 网格**，不再使用 J 压平：

- **θ / c 网格形状**: `(nb, ny, nz, nr)`，索引为 `θ[ib, iy, iz, ir]`
  - `ib`: 债券网格索引 (0 到 `nb_spg-1`)
  - `iy`: 收入网格索引 (0 到 `ny-1`)
  - `iz`: 聚合状态 `z` 的网格索引 (0 到 `nz_spg-1`)
  - `ir`: 利率 `r` 的网格索引 (0 到 `nr_spg-1`)
- **分布 G(b,y)**: 2D 张量，形状 `(nb, ny)`，表示在 (b, y) 上的概率质量
...
**policy_from_grid** 的接口约定：消费网格为 **4D numpy** `(nb, ny, nz, nr)`；在「使用训练好的策略」中一次性将 θ→c 及各网格转为 numpy 后传入闭包。

对于任意状态 `(b, y, r, z)`：

1. 找到对应的网格索引 `(ib, iy, iz, ir)`
2. 从消费网格取值：`c = grid[ib, iy, iz, ir]`（已为 softplus(θ)+c_min）
3. 由预算约束得 `b' = (1+r)b + yz - c`，并裁剪到 `[b_min, b_max]`
...
### 3.3 时间复杂度与训练时间（1000 epoch 参考）

实现见 [SRL/hugget.ipynb](SRL/hugget.ipynb) 及 `run_hugget_cluster.py`；校准采用 SRL Table 3：`N_epoch=1000`，`N_sample=512`，`T_horizon=50`，SPG 网格 `nb_spg=50`，`ny=3`，`nz_spg=10`，`nr_spg=10`。

**每 epoch 时间复杂度（量级）**：

- 单 epoch 内：`spg_objective` 跑 `N_sample` 条轨迹，每条 `T_horizon` 步。
- 每步主要操作：市场出清 `P_star`（在 `nr_spg` 个 r 上算储蓄）、分布更新 `update_G_pi_direct`（与 `nb_spg × ny`、软权重矩阵有关）、效用与梯度。
- 量级可写为：**O(N_sample × T_horizon × (nr_spg + nb_spg × ny × nb_spg))**，即每 epoch 与 **N_sample × T × (nr + nb²×ny)** 同量级。

**1000 epoch 训练时间（经验估计）**：

- 与硬件（CPU/GPU）、是否用 MPS/CUDA 强相关。
- 实际运行**较慢**：在常见单机上（如 M1/M2 或 x86），1000 epoch 往往需要 **数小时**（约 2–6 小时或更久）；400 epoch 约 1–2 小时量级。
- 精确时间以本地运行 `hugget.ipynb` 或 `python run_hugget_cluster.py --epochs 1000` 的实测为准；长时间训练建议用集群或 `--quick` 先做流程验证。
...
- **init_theta 数值稳定**: 初值由 `c = (1 - save_frac) * cash` 反解 `θ = log(exp(c - c_min) - 1)`（softplus 的反函数）。当 `c - c_min` 较大时 `exp(...)` 会溢出。代码中对 `x = c - c_min` 分段处理：`x > 20` 时用近似 `θ ≈ x`，否则用 `log(exp(clamp(x)) - 1)` 并 clamp 避免溢出。
- **SPG 网格**: `r_grid_t` 由 `np.linspace(r_min, r_max, nr_spg)` 直接生成；仅 `z` 从主网格用 `iz_spg` 抽样。
